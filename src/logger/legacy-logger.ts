// https://github.com/TypeStrong/ts-node?tab=readme-ov-file#missing-types
...
export class LpLogger {
  ...
  private static readonly _sensitiveHeaders = new Set([
    ...
    'x-kkr-portal-headers',
    'x-kkr-custom-cf-header'
  ])
  ...
  static readonly setup = (serviceName: string, config: LpLoggerConfig): void => {
    this._minLogLevel =
      LogLevel[process.env.MIN_LOG_LEVEL as keyof typeof LogLevel] ?? LogLevel.Information
    this._traceData = {
      ...
      serviceName,
      traceId:
        config?.traceId ?? process.env[LambdaTraceHeaderKey] ?? `[GENERATED]${uuidv4()}`,
  ...
  static logEvent (event: APIGatewayProxyEvent): void {
    this.log(LogLevel.Information, 'Request event:', this.createSanitizedTrace(event))
  }
  ...
  static getLogRecord (level: LogLevel, message: string, details?: any): LogRecord {
    const record: any = {
  ...
  static sanitizeQueryStringParameters (queries: StringMap | undefined): StringMap {
  ...
  static createSanitizedTrace (event: any): StringMap {
    const { path, httpMethod, queryStringParameters, headers, resource, requestContext } =
    ...
    return {
      ...
      path,
      resource,
      queryStringParameters: this.sanitizeQueryStringParameters(queryStringParameters),
      requestContext,
